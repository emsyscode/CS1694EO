...
/**********************************************************/
/*ATT:The model CS1694 use 12bits to fill memory map.     */
/**********************************************************/
//The CS1694 support 7 grids of 12 segments, but the panel need
//9 grids, by this reason they swapp the grids by segments and change
...
#define CS1694_in 7   // If 0 write LCD, if 1 read of LCD
#define CS1694_clk 8  // if 0 is a command, if 1 is a data0
#define CS1694_stb 9  // Must be pulsed to LCD fetch data of bus
...
void setup() {
    ...
    */
    ...
    //only here I active the enable of interrupts to allow run the test of CS1694
    //interrupts();             // enable all interrupts, is same as sei();
    sei();
}
void CS1694_init(void){
  ...
  // Configure CS1694 display (grids)
  cmd_with_stb(gridSegments); // cmd 1 // CS1694 is driver until 7 grids
  delayMicroseconds(1);
...
void cmd_without_stb(unsigned char a){
  ...
  //This don't send the strobe signal, to be used in burst data send
         for (mask = 00000001; mask>0; mask <<= 1) { //iterate through bit mask
           digitalWrite(CS1694_clk, LOW);
                 if (data & mask){ // if bitwise AND resolves to true
                    digitalWrite(CS1694_in, HIGH);
                 }
                 else{ //if bitwise and resolves to false
                   digitalWrite(CS1694_in, LOW);
                 }
          delayMicroseconds(5);
          digitalWrite(CS1694_clk, HIGH);
          delayMicroseconds(5);
         }
   //digitalWrite(CS1694_clk, LOW);
}
...
void test_WheelsOFF(void){
  ...
  //Command 3: Address Setting Command      //Without Strobe impulse
    digitalWrite(CS1694_stb, LOW);
    delayMicroseconds(1);
    cmd_with_stb(gridSegments); // cmd 1 // CS1694 is a drive of 7 grids
    ...
    cmd_with_stb(0b10000000);   // cmd 2 //Normal operation; Set pulse as 1/16
             //
                          ...
                          delayMicroseconds(1);
                            ...
                            //
                            cmd_without_stb(0b00000000); // "a" of all digits and symbol repeat
                            cmd_without_stb(0b00000001); // Symbol DVD & Wheel 8
                            
                            cmd_without_stb(0b00000000); // "b" of all digits and symbol wheel 
                            cmd_without_stb(0b00000001); // Symbol VCD & Whell 1
                            
                            cmd_without_stb(0b01000000); // "c" of all digits and symbol wheel
                            cmd_without_stb(0b00000001); // Symbol MP3 & Wheel 5
                            
                            cmd_without_stb(0b00000000); // "d" of all digits 
                            cmd_without_stb(0b00000001); // Symbol PBC & Wheel 4
                            
                            cmd_without_stb(0b00000000); // "e" of all digits
                            cmd_without_stb(0b00000001); // Symbol Play & Wheel 6
                            
                            cmd_without_stb(0b00000000); // "f" of all digits
                            cmd_without_stb(0b00000001); // Symbol Pause & Wheel 2
                            
                            cmd_without_stb(0b00000000); // "g" of all digits
                            cmd_without_stb(0b00000011); // Symbol Wheel 3 & 7
                            digitalWrite(CS1694_stb, HIGH);
                            cmd_with_stb((0b10001000) | 7); //cmd 4
                            delay(200);
}
void test_AllON(void){
  ...
  //Command 3: Address Setting Command      //Without Strobe impulse
    digitalWrite(CS1694_stb, LOW);
    delayMicroseconds(1);
    cmd_with_stb(gridSegments); // cmd 1 // CS1694 is a drive of 7 grids, here is used as 9 grids because they revert grids/segments
    cmd_with_stb(0b10000000);   // cmd 2 //Normal operation; Set pulse as 1/16
             //
                          digitalWrite(CS1694_stb, LOW);
                          delayMicroseconds(1);
                            ...
                            cmd_without_stb(0b00000011); // Symbol DVD & whell 8
                            //First two       .:
                            cmd_without_stb(0b11111111); // "b" of all digits and symbol wheel and also the ":"
                            cmd_without_stb(0b00000011); // Symbol VCD & whell 1
                            //First two       .:
                            cmd_without_stb(0b11111111); // "c" of all digits and symbol wheel and also the ":"
                            cmd_without_stb(0b00000011); // Symbol MP3 & whell 5
                            
                            cmd_without_stb(0b11111111); // "d" of all digits 
                            cmd_without_stb(0b00000011); // Symbol PBC & whell 4
                            
                            cmd_without_stb(0b11111111); // "e" of all digits
                            cmd_without_stb(0b00000011); // Symbol Play & wheel 6
                            
                            cmd_without_stb(0b11111111); // "f" of all digits
                            cmd_without_stb(0b00000011); // Symbol Pause & wheel 2
...
void test_panel_DVD(void){
  for (int x=0; x< 10; x++){
    digit=x;
          ...
          for (int i=0; i< 10;i++){
              delayMicroseconds(1);
              cmd_with_stb(gridSegments); // cmd 1 // CS1694 is a driver of 7 grids
              cmd_with_stb(0b10000000);   // cmd 2 //Normal operation; Set pulse as 1/16
              digitalWrite(CS1694_stb, LOW);
              delayMicroseconds(1);
              cmd_without_stb(0b11000000); // Grids of display... they have done the swap of this pins with segments
...
void clear_CS1694(void){
  /*
  Here I clean all registers 
  Could be done only on the number of grid
  to be more fast. The 12 * 3 bytes = 36 registers
  */
      for (int n=0; n < 14; n++){  // important be 10, if not, bright the half of wells./this on the CS1694 of 6 grids)
        //cmd_with_stb(gridSegments); // cmd 1 // CS1694 is fixed to 5 grids
...
void updateDisplay(void){
  //Note: This sequence is to a panel with CS1694 and 6 DSP de 7segm e 1 de digito 1
  //wheels and 8 predefined symbols. ("1.30.2 DP903002R" on PCB panel)
    write_panel_DVD( secsU, 5);// 
    write_panel_DVD( secsD, 4);// 
    write_panel_DVD( minutU, 3);// 
    write_panel_DVD( minutD, 2);// 
    write_panel_DVD( hourU, 7);// 
    write_panel_DVD( hourD, 1);// 
}
...
void readButtons(){
  ...
  //This don't send the strobe signal, to be used in burst data send
         for (int z = 0; z < 4; z++){
             //for (mask=00000001; mask > 0; mask <<= 1) { //iterate through bit mask
                   for (int h =8; h > 0; h--) {
                      digitalWrite(CS1694_clk, HIGH);  // Remember wich the read data happen when the clk go from LOW to HIGH! Reverse from write data to out.
                      delayMicroseconds(2);
              ...
              Serial.print(" - " );
                        ...
                        if (z==1){
                           if(array[4] == 1){
                             ...
                             //digitalWrite(CS1694_onGreen, !digitalRead(CS1694_onGreen));
                            }
                        } 
...
void write_Wheels(unsigned char segm, unsigned char grid){
  //Only to design the Wheels segments move!
                          switch (segm) {
                                 ...
                                 if (grid == 8){  //This define wich RUN only one time by update, can be any number of grid(segmente on this solution) 0~9.
                                  cmd_with_stb(gridSegments); // cmd 1 // CS1694 is a driver of 7 grids
                                  cmd_with_stb(0b10000000); // cmd 2 //Normal operation; Set pulse as 1/16
                                  digitalWrite(CS1694_stb, LOW);
                                  delayMicroseconds(1);
    ...
    led();
     ...
     for (uint8_t i = 0x00; i < 36; i++){
      segmWheels++;
              ...
              write_Wheels(wheels[segmWheels], 8); // Grid 1 run over digit 5 from right count, grid 0 is first digit after wheel
              delay(100);
     }
